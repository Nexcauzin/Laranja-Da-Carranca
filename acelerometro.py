...
def pegaAcelerometro(diretorio):
    with open(f'{diretorio}/acelerometro.csv', mode='w', newline='') as csv_file:
        MPU_Init()
        writer = csv.DictWriter(csv_file, fieldnames=['Roll', 'Pitch', 'AccX', 'AccY', 'AccZ'])
        writer.writeheader()
        ...
        while True:
            ...
            try:
                AngleRoll = math.atan(AccZ/AccY)*180/math.pi - 89.01
                AnglePitch = math.atan(AccZ/AccX)*180/math.pi + 85.82
            ...
            dados_float = list(map(float, [KalmanAngleRoll,KalmanAnglePitch, AccX, AccY, AccZ]))

            row_data = dict(zip(['Roll', 'Pitch', 'AccX', 'AccY', 'AccZ'], dados_float))
            #print(row_data)
